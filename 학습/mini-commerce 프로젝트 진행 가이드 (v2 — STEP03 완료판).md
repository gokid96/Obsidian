...
> 레퍼런스 WIL은 `docs/WIL/weekN/README.md`에 **주차별 1파일**로 쓴다. 본인은 `docs/devlog/`에 **주제별 1파일**로 쓴다 (주차가 아니라 주제 단위). 검색·재사용에 유리.

**✅ 레퍼런스 WIL 실제 작성 시점 (각 글의 끝 문장으로 확인):**

- week2: "REST Docs 배포하며 이번 주차를 마무리했다", "다음 주차부터는…" → **설계(STEP02) 전부 끝내고** 작성
- week3: "과제를 마무리하고 제출했다", "BP 받았다", 과제 PR 링크, "다음 주차부터는…" → **도메인 구현(STEP03) 전부 끝내고 제출 후** 작성
- week5: 제목이 "5, 6주차 - 동시성" → **2개 STEP을 묶어서**, 그 작업 후 작성

→ 즉 레퍼런스 패턴: ① **그 STEP 작업을 전부 끝내고(머지/제출 후)** 회고체로 작성, ② 한 회고에 그 기간 작업 전부 + 다음 예고 포함, ③ week5처럼 **연관된 2개 STEP을 묶기도** 함.

**작성 타이밍 (본인 확정):** "각 STEP 머지 직후"가 아니라 **그 STEP의 도메인을 전부 만든 후**에 쓴다 (레퍼런스와 동일). 진행 중에 미리 쓰면 아직 없는 코드(예: 주문 없는데 협력 얘기)를 쓰게 돼 모순이 생긴다. 회고엔 개념·결정 위주로 쓰고, 사소한 실수담은 굳이 넣지 않는다.

> STEP03처럼 주문(STEP04)과 엮인 주제("도메인 간 협력" 등)는 STEP03 회고에 넣지 않고, **주문까지 만든 뒤** STEP04 회고에서 다룬다. STEP03~04를 묶어서 한 번에 써도 된다 (week5 선례).

**기존 devlog 파일:**

- `이커머스 아키텍처 설계.md` — STEP02 설계 회고
- `DTO패턴이유.md` — DTO 설계 결정 (결정+이유 표, 담백)

**STEP03 devlog (초안 작성됨 — STEP04 후 보강 예정):**

- `DDD-lite&4Layer.md` — 클린 레이어드 정의, 엔티티=도메인 겸용(+한계+택한 이유), 4원칙, 4-Layer, DIP, DTO 흐름, Facade 생략
- `도메인설계원칙.md` — 정적 팩토리+PROTECTED, 검증은 도메인 내부, `cannotXxx()` 헬퍼, ERD 정렬(쿠폰), Product-Stock 분리, JPA 연관관계 최소화
- ⚠️ "도메인 간 협력(ID vs 객체 참조)" 섹션은 **STEP04(주문) 후로 미룸** — STEP03엔 주문이 없어 모순. DDD 용어("순수/라이트/분리형")는 "클린 레이어드/겸용"으로 정리 중.

**작성 방식:** 회고는 본인 생각이 핵심이라, Claude는 주제+뼈대만 제시하고 본인이 본인 언어로 채운다. `DTO패턴이유.md`처럼 표+짧은 산문이 본인 톤. (Claude가 직접 쓰지 않음.)

**STEP별 devlog 주제 (그 STEP 도메인 다 만든 후 작성):**

|STEP|주제(예시)|
|---|---|
|STEP03|클린 레이어드/4Layer, 도메인 설계 원칙 (위 2파일)|
|STEP04|도메인 간 협력(ID 참조 + 스냅샷), 주문/결제 통합 결정, 트랜잭션(@Transactional 위치), 인덱스(카디널리티/커서페이징)|
|STEP05|동시성(낙관vs비관 기준), 분산락 순서, Balance 리팩토링|
|STEP06|캐시 전략(Read Through/TTL 49h), Redis ZSET|
|STEP07|Facade→EDA 전환, Saga 보상|
|STEP08|Kafka(파티션키/Outbox/멱등성)|
|STEP09|부하테스트 + 전체 종합 회고|

> 핵심 결정 사항(예: 도메인 참조는 Long ID + 스냅샷, 레퍼런스 실제 구현 방식)은 그 STEP에서 코드로 구현한 뒤 회고에 기록.
