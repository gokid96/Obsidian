...
|STEP03|REST Docs 복구 (Balance, Coupon)|진행 중|작업 중 (`refactor/rest-docs-restore`)|

### 다음 작업 (즉시 진행)

**REST Docs 복구 PR 마무리** (`refactor/rest-docs-restore` 브랜치)

- BalanceControllerDocsTest 작성 완료
- CouponControllerDocsTest 작성 완료
- 커밋 + Push + PR + 머지 남음

**그다음: 이슈 #22 — 상품 비즈니스 로직 구현 및 단위 테스트**

- 브랜치명: `feat/step03-product-domain`
- STEP03 마지막 이슈
- **정석 흐름 적용**: Docs 테스트도 함께 작성, 항상 모든 테스트 통과 상태 유지
- ProductControllerDocsTest도 정식 구현 시 함께 갱신
...
- **AOP (Aspect-Oriented Programming)** — `@Transactional`, `@RestControllerAdvice`
...
- **Test-After Development (TAD)** — 본인이 쓰는 방식. 코드 먼저 → 테스트 나중
- **Mock-First Development** — mock으로 골격 만들고 실제 도메인 채워가는 방식
- **Walking Skeleton** — 전체 골격(엔드포인트~mock 응답)을 먼저 세우고 살 붙이기
- **Iterative Development** — STEP 단위로 점진적 진화
- **Continuous Refactoring** — 코드를 여러 번 다듬음 (레퍼런스 ~35%가 REFACTOR)
- **Always Green** — 빌드/테스트 항상 통과 상태 유지 (이슈 #22부터 적용)
- **Living Documentation** — Spring REST Docs로 테스트=문서

### 동시성/성능 패턴 (예정)

- **Optimistic Locking (낙관적 락)** — STEP05, 잔액
- **Pessimistic Locking (비관적 락)** — STEP05, 쿠폰/재고
...
- **Given-When-Then (BDD 스타일)** — 모든 테스트 메서드에 적용
...
1. 빨강: 실패하는 테스트 작성 (본체 없음/부족)
2. 초록: 테스트 통과시키기 위해 본체 작성
3. 리팩토링: 통과 유지하며 정리
...
- 본체 먼저 작성 (도메인, Controller)
- 테스트 나중 작성
- 본체 변경 시 테스트도 같이 갱신 (Always Green)
...
2. 테스트 통과시키려고 본체 작성? (TDD: 네, 본인: 아니오)
3. 테스트가 본체 설계 이끌었나? (TDD: 네, 본인: 아니오)

본인 방식은 **현업 80% 다수 방식**. 학습 단계에 적합. STEP09 후 여유 생기면 TDD 시도해볼 가치 있음.
...
- Docs: 정상 시나리오만 (검증 실패는 안 다룸. Controller Test에서 이미 다룸)
...
이미 작성된 `BalanceControllerDocsTest`, `CouponControllerDocsTest` 그대로 패턴 따라가기:
...
- 증상: STEP02에서 만든 Docs 테스트가 STEP03 본체 갈아엎으면서 깨짐
- 원인: 본체에 의존성 추가됐는데(`new XxxController(facade)`) Docs 테스트는 옛 코드(`new XxxController()`) 그대로
- **본질**: "미완성"이 아니라 "본체 변경 시 테스트 동기화 안 함"
- 해결 방법 2가지:
    - 빠른 진행: 삭제 (본인이 한 방식 — STEP09 후 새로 작성)
    - 정석: 본체 갈아엎을 때 같이 갱신 (이슈 #22부터 이 방식 적용)
- **레퍼런스 방식**: 만들었던 Docs 테스트 삭제 안 함. 본체 진화에 맞춰 같이 갱신해서 끝까지 살림
...
- 추가: 중복 커밋 합치기는 `squash` 사용
- 안전판: `git rebase --abort` 로 원상복구 가능
- 강제 push: `git push --force-with-lease` (단독 브랜치라 안전)
...
그다음 STEP03 이슈 #22 (Product 도메인) 진행할거야.
정석 흐름으로:
- 도메인 만들면서 Docs 테스트도 같이 작성
- 항상 모든 테스트 통과 상태 유지
- 커밋 단위 의식적으로 쪼개기
