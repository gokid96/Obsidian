...
---
하나하나 짚어가며 찾는 방식은 시간 복잡도가 O(N)입니다. 
O(N)이 허용하는 연산 횟수는 1,000만이므로 데이터의 개수가 1,000만 개 이하면 이 알고리즘은 사용해도 됩니다
...
| n의 크기          | 사용 가능한 최대 시간복잡도 | (1초) 연산 횟수  | 대표 알고리즘   |
| -------------- | --------------- | ----------- | --------- |
...
```
n의 크기가 최대 10일 경우, O(n!) 알고리즘을 써도 된다. 
왜냐하면 n=10일 때 10! = 3,628,800번 연산이라 1억 이하니까. 
O(n!) 복잡도를 가지는 대표 알고리즘은 순열 완전탐색이다.

n의 크기가 최대 500일 경우, O(n³) 알고리즘을 써도 된다. 
왜냐하면 n=500일 때 500³ = 125,000,000번 연산이라 1억 근처니까. (경계선 ⚠️) 
O(n³) 복잡도를 가지는 대표 알고리즘은 플로이드-워셜이다.
```
### 실전 사고 흐름
