...
|Method|Endpoint|설명|데이터소스|캐시|
|---|---|---|---|---|
|`POST`|`/search`|메시지 검색 (빠름)|MySQL (iapp-kor)|없음|
...
| Method | Endpoint                       | 설명                 | 데이터소스          |
| ------ | ------------------------------ | ------------------ | -------------- |
| `GET`  | `/mq-in`                       | MQSeriesToQueue 상태 | xaAMR REST API |
...
| 데이터소스         | 캐시    | 주기         |
| ------------- | ----- | ---------- |
| **xaAMR API** | 캐시 없음 | 5초마다 직접 호출 |
| **Redis**     | 캐시 없음 | 5초마다 직접 호출 |

| 토픽                             | 설명           | 브로드캐스트 주기 | 데이터소스         |
| ------------------------------ | ------------ | --------- | ------------- |
| `/topic/handler/prod1/status`  | PROD1 호스트 상태 | **5초**    | Redis + xaAMR |
| `/topic/handler/prod1/summary` | PROD1 요약     | **5초**    | Redis + xaAMR |
| `/topic/handler/prod2/status`  | PROD2 호스트 상태 | **5초**    | Redis + xaAMR |
| `/topic/handler/prod2/summary` | PROD2 요약     | **5초**    | Redis + xaAMR |
| `/topic/handler/test/status`   | TEST 호스트 상태  | **5초**    | Redis + xaAMR |
| `/topic/handler/test/summary`  | TEST 요약      | **5초**    | Redis + xaAMR |

```
[대시보드 열림] → WebSocket 구독 → 백엔드 5초마다 폴링 시작
                                         ↓
                              Redis 조회 + xaAMR 조회 (둘 다 캐시 없음)
                                         ↓
                              WebSocket으로 브라우저에 푸시
                                         ↓
                                    (5초 후 반복)

[대시보드 닫음] → WebSocket 해제 → 백엔드 폴링 중지
```
